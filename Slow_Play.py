...
class Player(BaseBot):
    ...
    def estimate_opp_turn_strength(self, current_state: PokerState) -> float:
        """
        Estimates opponent turn strength by mapping their flop bucket onto the
        play_threshold numeric scale. monster -> ~2x play_threshold, air -> 0.
        """
        SCALE_FACTOR = 2.0
    ...
    def estimate_opp_river_strength(self, current_state: PokerState) -> float:
        """
        Estimates opponent's river strength by averaging eval7 score across
        all ~44 possible second hole cards.
        """
        revealed_card = eval7.Card(current_state.opp_revealed_cards[0])
        board = [eval7.Card(c) for c in current_state.board]
        seen = (
            set(current_state.my_hand)
            | set(current_state.board)
            | {str(revealed_card)}
        )
    ...
    def evaluate_turn_equity(self, current_state: PokerState, skip_card: str = None) -> float:
        ...
        contribution multiplier applied. Returns average adjusted score.
        """
    ...
    def get_move(
        ...
        if current_state.street == 'auction':
            ...
            if tier == 'premium':
                v = random.uniform(0.20, 0.25)
            elif tier == 'strong':
                v = random.uniform(0.25, 0.30)
            elif tier == 'playable':
                v = random.uniform(0.20, 0.25)
            else:
    ...
    def is_dry_board(self, board) -> bool:
        """
        Returns True if board is dry (safe to slow play monsters).
        Dry  = paired board, no flush draw, low connectivity.
        Wet  = flush draw possible, connected ranks.
        """
    ...
    def _handle_flop(self, current_state: PokerState):
        ...
        if bucket == 'monster':
            # On dry boards, slow play 60% of the time to disguise strength
            if self.is_dry_board(current_state.board) and random.random() < 0.6:
                if current_state.can_act(ActionCheck):
                    return ActionCheck()
                return ActionCall()
            if current_state.can_act(ActionRaise):
    ...
    def _handle_turn(self, current_state: PokerState):
        ...
        if current_state.opp_revealed_cards:
            ...
            abs_raise_fraction = min(
                1.0,
                max(0.0, (our_score - self.play_threshold) / self.play_threshold)
            )
    ...
    def _handle_preflop(self, current_state: PokerState):
        """
        Pre-flop logic.
        Uses stack-based raise sizing (pot too small to be meaningful).
        """
        tier = self.classify_preflop_hand(current_state)
        my_chips = current_state.my_chips
        cost = current_state.cost_to_call
